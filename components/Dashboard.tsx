...
import { BookOpen, MessageCircle, HelpCircle, Lock, LogOut } from "lucide-react";
...
const Dashboard: React.FC<Props> = ({ onStart, onMenu, onLogout, activityData }) => {
  const [userName, setUserName] = useState("Ahmad");
  const [userEmail, setUserEmail] = useState("");

  const isMateriAccessed = activityData?.materi_accessed || false;
  ...
  const handleRestrictedMenu = (screen: Screen) => {
    if (!isMateriAccessed && (screen === "ANALYSIS" || screen === "EVALUATION")) {
      alert("Silakan akses menu Materi terlebih dahulu sebelum membuka Diskusi atau Kuis.");
      return;
    }
    onMenu(screen);
  };
      ...
      <main className="flex-1 pb-24 px-6">
        <header className="pt-8 pb-2">
          <div className="flex justify-between items-start">
            ...
            <button
              onClick={onLogout}
              className="p-3 rounded-xl bg-white dark:bg-zinc-900 shadow-sm border border-gray-100 dark:border-zinc-800 text-gray-400 hover:text-red-500 transition-colors active:scale-95"
              title="Logout"
            >
        ...
        <section className="py-2">
          ...
          <div className="grid grid-cols-3 gap-5">
            ...
            <button
              onClick={() => handleRestrictedMenu("ANALYSIS")}
              className={`flex flex-col items-center gap-4 group transition-all duration-200 ${!isMateriAccessed ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`}
            >
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br from-orange-100/80 to-orange-50 flex items-center justify-center text-orange-600 shadow-sm transition-all duration-200 ${isMateriAccessed ? 'group-hover:from-orange-500 group-hover:to-orange-600 group-hover:text-white group-hover:shadow-lg group-hover:shadow-orange/20' : ''}`}>
                {isMateriAccessed ? <MessageCircle className="w-10 h-10" /> : <Lock className="w-10 h-10" />}
              </div>
              <span className="text-sm font-semibold text-gray-900 dark:text-gray-200 text-center">
                Diskusi
              </span>
            ...
            <button
              onClick={() => handleRestrictedMenu("EVALUATION")}
              className={`flex flex-col items-center gap-4 group transition-all duration-200 ${!isMateriAccessed ? 'opacity-50 cursor-not-allowed' : 'active:scale-95'}`}
            >
              <div className={`w-20 h-20 rounded-2xl bg-gradient-to-br from-amber-100/80 to-amber-50 flex items-center justify-center text-amber-600 shadow-sm transition-all duration-200 ${isMateriAccessed ? 'group-hover:from-amber-500 group-hover:to-amber-600 group-hover:text-white group-hover:shadow-lg group-hover:shadow-amber/20' : ''}`}>
                {isMateriAccessed ? <HelpCircle className="w-10 h-10" /> : <Lock className="w-10 h-10" />}
              </div>
              <span className="text-sm font-semibold text-gray-900 dark:text-gray-200 text-center">
                Kuis
              </span>
